...
class Merger:
    ...
    def _create_updated_models(self, base_model, input_merge_spaces):
        """Dynamically creates deltas, but never from Loras."""
        ...
        for i, model in enumerate(self.models):
            ...
            # Prevent subtract if either the current model or the base model is a Lora
            if (i < len(input_merge_spaces) and
                    input_merge_spaces[i] == sd_mecha.recipe_nodes.MergeSpace.DELTA and
                    model.model_type.identifier != "lora" and  # Compare the identifier directly
                    base_model.model_type.identifier != "lora"):  # Compare the identifier directly
    ...
    def _slice_updated_models_if_needed(self, updated_models, mecha_merge_method):
        """Slices updated_models if the method doesn't accept *args and the number of models doesn't match."""
        ...
        if mecha_merge_method.get_model_varargs_name() is None and num_models != expected_num_models:
            # Check if the method implicitly requires a base model for delta operations
            requires_base_for_delta = mecha_merge_method.get_return_merge_space(
                [sd_mecha.recipe_nodes.MergeSpace.BASE] * expected_num_models
            ) == sd_mecha.recipe_nodes.MergeSpace.DELTA

            # Suppress the warning if the base model is included and implicitly required
            if requires_base_for_delta and self.cfg.base_model_index >= expected_num_models:
                logger.debug(
                    f"Merging method '{self.cfg.merge_mode}' implicitly requires a base model for delta operations. Suppressing model count warning."
                )
            else:
                updated_models = updated_models[:expected_num_models]
                logger.warning(
    ...
    def merge(
        ...
        # Create updated models list, handling both Loras and base models
        updated_models = self._create_updated_models(base_model, input_merge_spaces)

        # Slice updated_models if necessary
        sliced_models = self._slice_updated_models_if_needed(updated_models, mecha_merge_method)

        # Merge the models
        merged_model = self._merge_models(updated_models, all_hypers, cache)

        # Handle delta output if needed
        merged_model = self._handle_delta_output(merged_model, mecha_merge_method, sliced_models, base_model)
...
I'll compare the working but no lora code to the not working but lora code. 1st snippet will always be the working old state, 2nd the not working current
...
elif i < len(input_merge_spaces) and input_merge_spaces[i] == sd_mecha.recipe_nodes.MergeSpace.DELTA:
```
...
if mecha_merge_method.get_model_varargs_name() is not None and all(
    ...
    updated_models = [sd_mecha.subtract(model, base_model) for i, model in enumerate(updated_models) if
                      i != self.cfg.base_model_index]
```
...
class Merger:
    ...
    def merge(
        ...
        if mecha_merge_method.get_model_varargs_name() is None and num_models != expected_num_models:
            ...
            # Suppress the warning if the base model is included and implicitly required
            if requires_base_for_delta and self.cfg.base_model_index >= expected_num_models:
                logger.debug(f"Merging method '{self.cfg.merge_mode}' implicitly requires a base model for delta operations. Suppressing model count warning.")
            else:
                updated_models = updated_models[:expected_num_models]  # Slice self.models
                logger.warning(
                    f"Merging method '{self.cfg.merge_mode}' expects {expected_num_models} models, but {num_models} were provided. Using the first {expected_num_models} models."
